...
TARGET_PHRASE = 'hello world!'  # target DNA
...
class GA(object):
    ...
    def select(self):
        """从pop中重复抽样，根据概率筛选后代，优质后代会重复被选走"""
        fitness = self.get_fitness() + 1e-4  # add a small amount to avoid all zero fitness
        idx = np.random.choice(np.arange(self.pop_size), size=self.pop_size, replace=True, p=fitness / fitness.sum())
        return self.pop[idx]

    def crossover(self, parent, pop):
        if np.random.rand() < self.cross_rate:  # 满足交叉概率，开始交叉
            i_ = np.random.randint(0, self.pop_size, size=1)  # select another individual from pop
            cross_points = np.random.randint(0, 2, self.DNA_size).astype(np.bool)  # 选出交叉的位置，这里不同任务限制不同
